...
class SVM4342 ():
    ...
    def fit (self, X, y):
        oneCon = np.ones(X.shape[0])
        reShaper = oneCon.reshape(X.shape[0], 1)
        Xtil = np.hstack((X, reShaper))

        # TODO change these -- they should be np.arrays representing matrices or vectors
        yArr = (y*-1)[:,None]
        G = yArr*Xtil
        identityMat = np.identity(X.shape[1]+1)
        P = identityMat
        zeroCon = np.zeros(X.shape[1]+1)
        q = zeroCon
        oneConMat = np.ones(X.shape[0])*-1
        h = oneConMat
        ...
        x = np.array(sol['x'])
        flattenArr = (x[:-1]).flatten()
        self.w = flattenArr  # TODO change this
        self.b = x[-1]  # TODO change this

    # Given a 2-D matrix of examples X, output a vector of predicted class labels
    def predict (self, x):
        transposeW = np.transpose(self.w)
        dotProd = x.dot(transposeW) + self.b
        dotProd[dotProd < 0] = -1
        dotProd[dotProd > 0] = 1
        return np.transpose(dotProd)  # TODO fix
